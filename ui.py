...
layoutprefile = [
    [sg.Text('Select two dataset:')],
    [sg.Text('Set 1'), sg.InputText(), sg.FileBrowse()],
    [sg.Text('Set 2'), sg.InputText(), sg.FileBrowse()],
    [sg.Output(size=(61, 5))],
    [sg.Submit('Proceed'), sg.Cancel('Exit')]
]

window = sg.Window('Compare', layoutprefile)
    ...
    if event in (None, 'Exit', 'Cancel'):
        ...
        if file1 and file2:
            ...
            if not file1test and file1test is not None:
                ...
                print('Error: File format currently not supported. At the moment only csv and xlsx files are supported.')
                isitago = 0
            elif file1 == file2:
                ...
                try:
                    ...
                    elif re.findall('/.+?/.+\.(.+)',file1)[0] in  ['xlsx']:
